...
const mockQuestionData: QnA[] = [
  ...
  {
    question:
      "Hur kommer jag i kontakt med er i frågor som gäller annat än att få stöd?",
  ...
  },
  {
    ...
    answer:
      "Tyvärr riktar vi oss endast till döva och hörselskadade barn och ungdomar i åldrarna 7-21. Det finns dock andra jourer och platser där andra barn och vuxna kan få stöd. Det finns ibland möjlighet att chatta. Du finner en fullständig lista här:http://unizon.se/hitta-jour",
  },
  {
    question:
      "Hur kan jag göra om jag vill att BOUJT ska besöka min skola, arbetsplats eller förening?",
    answer:
      "Kul, tycker vi! Ta kontakt med de ansvariga, t.ex. din rektor, och be hen att mejla oss på info@boujt.se.",
  },
];

// TODO: Move this to it's own module? Will probably only be used here though...
const QuestionAndAnswer: React.FC<{ qna: QnA; idx: number }> = ({
  qna,
  idx,
}) => {
  return (
...
const OmOss: NextPage = () => {
  ...
  const questionsSecondHalf = mockQuestionData.slice(
    Math.ceil(offset),
    mockQuestionData.length
  );
    ...
    <BoujtTemplate gap={100}>
      ...
      >
        <Flex flexDir={"column"} flex={3}>
          ...
          <Text>
            Välkommen till BOUJT - Barn- och Ungdomsjour på Teckenspråk! Här får
            du hjälp och stöd på svenskt teckenspråk efter skoltid. Vi erbjuder
            bland annat stödsamtal med döva och hörselskadade syssnare samt
            information om kroppen, tankar och känslor. Vi har även en frågelåda
            där du kan ställa frågor.
            <br />
            <br />
            Du behöver aldrig oroa dig för att andra ska få veta att du
            kontaktat oss. Vi berättar inte för någon. Hos oss kan du vara helt
            anonym. I videochatten kan du välja vilken volontär du vill prata
            med, och du kan textchatta istället om du inte vill att volontären
            ska se dig. Du kan prata om vad du vill! Är du glad eller ledsen?
            Vill du berätta om något kul som hänt eller något som känns tungt
            och jobbigt? Har du kanske en fundering? Det spelar ingen roll. Läs
            mer om anonymitet här!
          </Text>
        </Flex>
      ...
      >
        <Flex flexDir={"column"} justifyContent={"center"} gap={"40px"}>
          ...
          <Text>
            Det är ett ord och tecken som betyder att man lyssnar fast med
            synen. Därför tecknas det som “lyssna” men med handen ovanför ögonen
            istället. Det är därför våra volontärer kallas för “syssnare”.
          </Text>
        </Flex>
        ...
        >
          ...
          <Flex flexDir={"column"}>
            {questionsSecondHalf.map((qna, idx) => {
              return (
                <QuestionAndAnswer qna={qna} idx={idx + Math.floor(offset)} />
              );
            })}
          </Flex>
        </Flex>
